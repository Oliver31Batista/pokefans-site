...
const Pokemon = () => {
  ...
  const obtenerDatos = () => {
    Axios.get(`https://pokeapi.co/api/v2/pokemon/${pokemonName}`).then((respuesta) => {
      setPokemon({
        name: pokemonName, 
    ...
    <div>
      ...
      <input 
        ...
        <div className="pokemon">
          {!pokemonChosen ? (<h1>busca un pokemon</h1>) : (<h1>{pokemon.name}<img src={pokemon.img} alt={pokemon.name}/></h1>)}
        </div>
    </div>
  )
}
